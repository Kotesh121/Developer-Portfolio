...
CERTIFICATION_DATE_PATTERN = re.compile(r"^\d{4}(?:-\d{2})?(?:-\d{2})?$")
CERTIFICATION_ID_PATTERN = re.compile(r"^cert-[A-Za-z0-9_-]{1,95}$")
...
app.add_middleware(
    ...
    # Preview deployments are convenient locally, but a production API must not
    # grant credentialed access to every Vercel project.
    allow_origin_regex=(
        None
...
def _certification_text(value, field: str, max_length: int, *, required: bool = False) -> str:
    """Normalize untrusted certification input before it reaches the database."""
    if value is None:
        value = ""
    if not isinstance(value, str):
        raise HTTPException(status_code=422, detail=f"Certification {field} must be text")
    value = value.strip()
    if required and not value:
        raise HTTPException(status_code=422, detail=f"Certification {field} is required")
    if len(value) > max_length:
        raise HTTPException(status_code=422, detail=f"Certification {field} must be {max_length} characters or fewer")
    return value

def _certification_url(value, field: str) -> str:
    value = _certification_text(value, field, 500)
    if value and not (value.startswith("/") or re.match(r"^https?://", value, re.IGNORECASE)):
        raise HTTPException(status_code=422, detail=f"Certification {field} must be an HTTPS/HTTP URL or site-relative path")
    return value

def normalize_certifications(payload) -> list[dict]:
    """Validate the full list before replacing it, ensuring atomic portfolio saves."""
    if not isinstance(payload, list):
        raise HTTPException(status_code=422, detail="certifications must be an array")
    if len(payload) > 100:
        raise HTTPException(status_code=422, detail="A maximum of 100 certifications is allowed")

    normalized, seen_ids = [], set()
    for position, record in enumerate(payload):
        if not isinstance(record, dict):
            raise HTTPException(status_code=422, detail="Each certification must be an object")
        cert_id = record.get("id") or generate_id("cert")
        if not isinstance(cert_id, str) or not CERTIFICATION_ID_PATTERN.fullmatch(cert_id):
            raise HTTPException(status_code=422, detail="Certification id is invalid")
        if cert_id in seen_ids:
            raise HTTPException(status_code=422, detail="Certification ids must be unique")
        seen_ids.add(cert_id)

        issued_date = _certification_text(record.get("issuedDate", record.get("date", "")), "issued date", 20)
        expiration_date = _certification_text(record.get("expirationDate", ""), "expiration date", 20)
        for label, date_value in (("issued date", issued_date), ("expiration date", expiration_date)):
            if date_value and not CERTIFICATION_DATE_PATTERN.fullmatch(date_value):
                raise HTTPException(status_code=422, detail=f"Certification {label} must use YYYY, YYYY-MM, or YYYY-MM-DD")

        skills = record.get("skills", [])
        if not isinstance(skills, list) or len(skills) > 30:
            raise HTTPException(status_code=422, detail="Certification skills must contain at most 30 items")
        clean_skills = []
        for skill in skills:
            clean_skill = _certification_text(skill, "skill", 60)
            if clean_skill and clean_skill not in clean_skills:
                clean_skills.append(clean_skill)

        try:
            display_order = int(record.get("displayOrder", position))
        except (TypeError, ValueError):
            raise HTTPException(status_code=422, detail="Certification display order must be a whole number")
        if not 0 <= display_order <= 10000:
            raise HTTPException(status_code=422, detail="Certification display order must be between 0 and 10000")

        normalized.append({
            "id": cert_id,
            "title": _certification_text(record.get("title"), "title", 200, required=True),
            "issuer": _certification_text(record.get("issuer"), "issuer", 150),
            "credential_id": _certification_text(record.get("credentialId"), "credential ID", 150),
            "issued_date": issued_date,
            "expiration_date": expiration_date,
            "credential_url": _certification_url(record.get("credentialUrl", record.get("url", "")), "verification URL"),
            "image_url": _certification_url(record.get("imageUrl", record.get("image", "")), "image URL"),
            "description": _certification_text(record.get("description"), "description", 4000),
            "skills": clean_skills,
...
def startup_event():
    if settings.ENVIRONMENT.lower() == "production":
        if settings.SESSION_SECRET == "portfolio-admin-session-secret-2024" or len(settings.SESSION_SECRET) < 32:
            raise RuntimeError("SESSION_SECRET must be a unique, 32+ character production secret")
...
async def save_portfolio(request: Request, db: Session = Depends(get_db)):
    ...
    certifications_to_save = normalize_certifications(body.get("certifications", []))
    ...
    for idx, c in enumerate(certifications_to_save):
